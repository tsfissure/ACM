...
template < int N, int MOD = -1 > struct Matrix {
  Matrix(bool isUnit = false) {
    memset(m, 0, sizeof(m));
    if(isUnit) { for(int i = 0; i < N; ++i) { m[i][i] = 1; } }
  }
...
FibMatrix U, T, sum[N << 1], lazy[N << 1];
...
inline void pushUp(int l, int r) {
  sum[CID] = (sum[LID] + sum[RID]) * lazy[CID];
}

inline void build(int l, int r) {
  lazy[CID] = FibMatrix(true);
  if(l >= r) {
...
inline FibMatrix query(int l, int r, int ql, int qr) {
  ...
  if(l >= ql && r <= qr) {
    ...
    return (query(LSON, ql, qr) + query(RSON, ql, qr)) * lazy[CID];
  }
}
